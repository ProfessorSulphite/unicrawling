...
def resolve_source_cap(max_sources: Optional[int] = None) -> int:
    ...
    An explicit override wins, but is strictly clamped to config.notebooklm_max_sources_limit.
    """
    hard_limit = getattr(config, "notebooklm_max_sources_limit", 300)
    cap = min(max_sources or config.max_sources_per_notebook, hard_limit)
    if cap < 1:
